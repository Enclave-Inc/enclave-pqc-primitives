//! ML-DSA-65 (FIPS 204) digital signatures — NIST Category 3.
//!
...
/// Expanded ML-DSA-65 signing key length (FIPS 204 `sk`).
...
const PCT_MESSAGE: &[u8] = b"enclave-pqc-sig65-pct-v1";
...
pub fn generate_keypair() -> Result<KeypairOutput> {
    ...
    Ok(KeypairOutput {
        keypair,
        usage: usage("sig65_generate_keypair"),
    })
}
...
pub fn keypair_from_seed(seed: &[u8]) -> Result<KeypairOutput> {
    ...
    Ok(KeypairOutput {
        keypair,
        usage: usage("sig65_keypair_from_seed"),
    })
}
...
pub fn expanded_secret_key(secret_key: &[u8]) -> Result<(Vec<u8>, CryptoUsageRecord)> {
    ...
    Ok((
        ...
        usage("sig65_expanded_secret_key"),
    ))
}
...
mod tests {
    ...
    #[test]
    fn sizes_match_fips_204_category_3() {
